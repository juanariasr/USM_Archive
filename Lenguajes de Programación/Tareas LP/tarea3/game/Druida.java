...
public class Druida extends Jugador{
    ...
    Actualiza las estadisticas del jugador generico a las especificas del tipo druida
    Constructor*/
    ...
    */
    public int ataque(){
        ...
        int melee_damage = ((get_fuerza()+Math.round(get_inteligencia())/3)) + Math.max(Math.round(get_energia()/3), Math.round(get_mana()/2));
    ...
    */
    public int hechizo(){
        ...
        int spell_damage = ((get_fuerza()+Math.round(get_inteligencia())/3)) + Math.max(Math.round(get_energia()/2), Math.round(get_mana()/3));
        set_mana(get_mana()-3);
        if(get_mana()-3 < 0){
    ...
    */
    public void subir_experiencia(int xp_ganada) {
        ...
        if (nuevo_level_actual != level_actual){
            System.out.println(get_nombre() + ": OMG! subi de nivel " + level_actual + " a nivel " + nuevo_level_actual);
        }
        while (nuevo_level_actual != level_actual){
            set_max_vida(get_max_vida() + nuevo_level_actual);
            set_fuerza(get_fuerza() + nuevo_level_actual);
            set_inteligencia(get_inteligencia() + nuevo_level_actual);
            set_max_energia(get_max_energia() + nuevo_level_actual);
            set_max_mana(get_max_mana() + nuevo_level_actual);
