...
import './EmparejarTimbre.css'
import NavBar from "../../barra/NavBar"
import { useNavigate } from 'react-router-dom';

const baseUrl = 'http://localhost:8000/';

export const EmparejarTimbre = ({ timbreSelected }) => {
    
    const [tasks, setTasks] = useState(timbreSelected);

    const navigate = useNavigate();

    function changeTipo() {
        for (var i in tasks) {
            if (tasks[i].tipo == "Percusion") {
                    tasks[i].tipo = 4;
                }
            if (tasks[i].tipo == "Cuerda") {
                    tasks[i].tipo = 3;
                }
            if (tasks[i].tipo == "Viento") {
                    tasks[i].tipo = 2;
            }
        }
     }
    ...
    return (
        <>
            ...
            <h1>
                Sonidos según timbre musical &nbsp;
            </h1>
            <p>
                Reproduce el audio de las imágenes y deslízalas a la columna que les corresponda según su timbre musical, 
                estos pueden ser instrumentos de viento, cuerda o percusión. ¡Mucho éxito!
            </p>
