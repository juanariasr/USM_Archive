...
import './EmparejarDuracion.css';
import { useNavigate } from 'react-router-dom';

const baseUrl = 'http://localhost:8000/'

export const EmparejarDuracion = ({ duracionSelected }) => {
    
    const [tasks, setTasks] = useState(duracionSelected);

    const navigate = useNavigate();

    function changeTipo() {
            for (var i in tasks) {
                if (tasks[i].tipo == "Largos") {
                        tasks[i].tipo = 2;
                    }
                if (tasks[i].tipo == "Cortos") {
                        tasks[i].tipo = 3;
                    }
                }
        }
    ...
    return (
        <>
            ...
            <p>
                Reproduce el audio de las imágenes y deslízalas a la columna que les corresponda según su duración, 
                estos pueden ser sonidos largos o sonidos cortos. ¡Mucho éxito!
            </p>
            <br/>
