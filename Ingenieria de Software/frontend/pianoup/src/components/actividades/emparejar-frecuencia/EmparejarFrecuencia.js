...
import './EmparejarFrecuencia.css';
import NavBar from "../../barra/NavBar";
import { useNavigate } from 'react-router-dom';

const baseUrl = 'http://localhost:8000/';

export const EmparejarFrecuencia = ({ frecuenciaSelected }) => {
    
    const [tasks, setTasks] = useState(frecuenciaSelected);

    const navigate = useNavigate();

    function changeTipo() {
        for (var i in tasks) {
            if (tasks[i].tipo == "Graves") {
                    tasks[i].tipo = 2;
                }
            if (tasks[i].tipo == "Agudos") {
                    tasks[i].tipo = 3;
                }
            }
    }
    ...
    return (
        <>
            ...
            <p>
                Reproduce el audio de las imágenes y deslízalas a la columna que les corresponda según su altura, 
                estos pueden ser sonidos graves o agudos. ¡Mucho éxito!
            </p>
            <br/>
